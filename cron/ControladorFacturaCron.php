...
require_once 'StampService.php';
require 'fpdf.php';
require 'NumeroALetras.php';

use SignService as Sellar;
use StampService as StampService;
...
class ControladorFacturaCron {
    ...
    private function getDatosReceptor($cid) {
        ...
        foreach ($sine as $dactual) {
        	$nombre = $dactual['nombre'] . ' ' . $dactual['apaterno'] . ' ' . $dactual['amaterno'] . '-' . $dactual['nombre_empresa'];
            $rfc = $dactual['rfc'];
            ...
            $datos .= "$nombre</tr>$rfc</tr>$razonsocial</tr>$clvreg</tr>$codpos</tr>$direccion";
        }
        return $datos;
    }
    ...
    function SelloXML($doc, $rfc) {
        ...
        $xmlFile = $doc;
        $carpeta = $folder . 'temporal/' . $rfc . '/';
        $xslFile = $folder . "cron/cadenaoriginal_3_3.xslt";
        $xml = new DOMDocument("1.0", "UTF-8");
        $xml->loadXML($xmlFile);
        $xsl = new DOMDocument();
        $xsl->load($xslFile);
        ...
        $fichero = $folder . "cron/cadenaOriginal.txt";
